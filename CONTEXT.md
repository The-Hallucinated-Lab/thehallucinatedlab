...
- [GAP-12]: `CLAUDE.md` §19.1 (added by PR #52) states that the dictionary "is brought over by `node scripts/sync-dictionary.js <path> [--check]`" and "is not hand-edited here". **That script does not exist** — not in `scripts/`, not in `package.json`, not on `main`. So the rule has no mechanism behind it: there is currently no way to bring dictionary changes over *except* by editing `dictionary/` in this repository, which the same sentence forbids. Until the script exists or the sentence is corrected, the protocol's authoritative addendum describes tooling the repository does not have, and an agent following it will run a command that fails.
