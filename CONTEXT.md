...
- **Trigger Event:** AI Edit
- **Author/Agent:** Claude (Master Orchestrator) for 06pratyush
- **Target Subsystem:** `dictionary.html` (new), `pages.css`, `script.js`, 27 pages carrying the navbar, `sitemap.xml`, `sitemap.html`, `llms.txt`, `llms-full.txt`
- **Intent:** Publish a Dictionary page and give it a live top-level nav entry between Media and Sitemap. Two sections, exactly as asked: **Artificial Intelligence** (18 terms, Agent through Vector database) and **Software Engineering** (16 terms, API through Technical debt). Each entry is one short definition followed by the part that is commonly got wrong, which is the only reason a lab glossary beats a search result. Every term carries its own `#term-<slug>` anchor so a single definition can be linked and lands highlighted (`.dict-entry:target`).
- **Bugs/Gaps Addressed:** None fixed, none opened. Worth noting it walks straight into [GAP-04]: the nav entry is one line, and shipping it meant a mechanical edit to 27 files. Done with a throwaway script run from the scratchpad rather than by hand — the script is not committed, since a one-off migration in the repo is a liability the next reader has to evaluate.
- **Context Modifications:** New page uses the existing `page-main` / `page-hero` / `media-section` / `container` / `section-title` skeleton; new CSS is one `.dict-*` block appended to `pages.css` with no new custom property. No new script, image, dependency or origin — the page ships `theme.js` and `script.js` and nothing else, so the CSP and the per-page JS budget are untouched. One `NAV_ICONS` entry added: a closed volume with three ruled lines cut out of the cover, the inner rectangles wound against the outer one so they render as holes under the default nonzero fill — the same trick the SLM chip already uses. Media owns the open book, and two books in one bar read as one section split in half. Discoverability surface updated in the same commit per RULE-03: `sitemap.xml` (priority 0.7), a new **Reference** section in `sitemap.html`, a `## Pages` line in `llms.txt`, and a full `## Page: Dictionary` section in `llms-full.txt` — which also required correcting the standing sentence that said the nav has *seven* entries.
- **Deliberate choices:** (1) **Live, not dev-gated.** The other recent sections (SLM, Certification, Consultancy, and Data, which landed on `main` while this branch was open) sit behind `data-status="dev"` because they describe work still on the bench. A glossary is finished the moment it is accurate, has nothing pending behind it, and was asked for as a live tab, so it ships indexable — which is also why it had to enter `sitemap.xml` and both `llms` files rather than staying out of them the way `slm.html` does. (2) **JSON-LD is a `DefinedTermSet` listing name and anchor only, not name and definition.** Repeating all 34 definitions in the head would double the maintenance surface and, under RULE-05, would be the exact shape that drifts into dishonest markup the first time a definition is reworded on the page but not in the graph. Names and anchors cannot drift silently — a renamed term breaks its own anchor visibly. (3) **No search box, no A–Z filter, no per-term copy button.** 34 entries in two alphabetical grids are faster to scan than to filter, and every one of those would be new JavaScript against the per-page budget for a page whose job is to be read. (4) `.dict-section { padding: 0 }` is deliberate and load-bearing: these are `<section>` elements nested inside one, and the global `section { padding: 6rem 2rem }` in `styles.css:263` otherwise indents the term grid out of alignment with the container the rest of the page lines up against.
- **Verification:** `npm run check` green (eslint clean, 370/370 `node --test`, spec in sync). Rendered in Chromium at 1440px and 390px, in both themes: nav glyph fills correctly beside the Media book, mobile overlay lists Dictionary between Media and Sitemap, `#term-rag` scrolls to a highlighted entry, no console errors.
