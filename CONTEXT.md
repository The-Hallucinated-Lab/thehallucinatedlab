...
- [GAP-10]: `dictionary/data/search-index.json` is now committed here, but it is *generated* in `06pratyush/ai_dictionary_thl` from that repo's corpus. There is no sync script and no test comparing the two, so adding a term upstream leaves this copy stale and the new term unsearchable on the live site. `spec/manifest.json` solves the same problem with `npm run spec:sync`; the dictionary corpus has no equivalent.
