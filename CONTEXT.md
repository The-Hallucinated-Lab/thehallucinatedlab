...
- **Identifier note (2026-08-16):** the timeline entries merged from PR #49 use **[GAP-08]** to mean *the dictionary's search is dead in production*. That gap was announced in those entries but never added to this section, and it is **now fixed** — see the `fix(dictionary)` entry in section 5, which commits the missing `dictionary/data/search-index.json`. This section's [GAP-08] is the complexity gap below, which is a different thing. Section 4 is authoritative; when the two disagree, this list wins.
