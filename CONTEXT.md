...
- [GAP-08]: A tool's `status` is a label, not a gate — **nothing filters on it**, by decision (see the 2026-08-09T22:15:00Z entry). Every tool in `spec/manifest.json` is offered to every visitor on both runtimes, so a `"dev"` tool is listed in the assistant's capability reply and matched by the intent parser like any other. The live consequence today: a visitor who asks for `link` is told it "runs in the Python package" and handed a `thl link ...` command that exits with `ToolNotFound`, because nothing implements it. The manifest `$comment`, the tool's `summary` and its `description` all state plainly that it is not built, which is the only thing currently keeping that exchange honest. **Prose is the sole protection here and no test enforces it** — a future `"dev"` tool whose summary reads as finished would ship a working-sounding promise with no failing build. If dev tools ever need to be genuinely unreachable again, the chokepoint is `loadManifest()` in `toolkit.js`, which every browser consumer shares.
...
- **Trigger Event:** AI Edit
- **Author/Agent:** Claude (Claude Code) for 06pratyush
- **Target Subsystem:** `data.html` (new), `link.html` (new), `spec/manifest.json`, `toolkit.js`, `script.js`, `tools.html`, all 27 navbars, `test/dev-mode.test.js`, `test/toolkit.test.js`
- **Intent:** Add two things behind dev mode: a **Data** section with its own page and nav entry, and a **`link`** tool registered in the tool spec. The `link` half is spec-only on purpose — the contract is written before the implementation, which is the order this repository already works in.
- **Bugs/Gaps Addressed:** Closes a latent one nobody had hit yet. `spec/manifest.json` has carried a `status` field since it was written, and its own `$comment` promised that `"dev"` tools are "visible only in dev mode and never shown to a visitor" — but **no code read the field.** All eleven tools were `live`, so the promise had never been exercised. `link` is the first `dev` tool, and shipping it without a filter would have made the comment false the moment it merged.
- **Why that mattered more than it looks:** a dev tool with no card on any page is still reachable. `nlp.js` scores *every* tool in the manifest against whatever a visitor types and never consults a page, and `interface.js` lists every tool under "here is what I can run, right here in the page". So the leak path was the Assistant, not the navigation — and hiding a card would have looked like a fix while changing nothing.
- **Context Modifications:**
  - `toolVisible()` and `visibleTools()` in `toolkit.js`'s pure block, applied inside `loadManifest()`. That fetch is the single chokepoint every browser consumer goes through — `interface.js`, the parser it feeds, the argument tables — so one filter covers all of them and the next consumer is covered without knowing the rule exists. A tool with no `status` is live, matching every entry written before the field existed.
  - `data.html` — Data section, `noindex`, with the `Data` nav entry added to all 27 navbars in the same position, a `NAV_ICONS` glyph in `script.js`, and an entry in `DEV_ONLY_PAGES`. No new CSS: it reuses the `slm-*` card family, which already carries the `pending`/`queued` status chips this page needed.
  - `link.html` + the `link` manifest entry (`status: "dev"`, `runtimes: ["python"]`, four params). A dev-marked gateway card on `tools.html` and a dev-marked link from `data.html` are its two routes in.
  - `npm run spec:sync` run, so `python/thehallucinatedlab/data/manifest.json` stays byte-identical.
- **On reading the mode from storage rather than the DOM:** `toolkit.js` reads `localStorage['thl_mode']` directly instead of the `data-mode` attribute `script.js` sets. Both files load with `defer` and nothing orders them against each other, so reading the DOM would make tool visibility depend on which script happened to run first — and the failure that produces is a dev tool appearing for a visitor, which is the one outcome the whole mechanism exists to prevent. The cost is that `MODE_KEY` is now written in two files with no shared module between them; a test asserts the two strings still match, because that drift would show up as a feature quietly not working rather than as a failure.
- **Caching note for whoever changes the toggle:** `loadManifest()` caches the *filtered* manifest. That is only safe because `toggleMode()` reloads the page. If the toggle ever becomes an in-place repaint, this cache goes stale and has to be keyed by mode — the comment at the filter says so too, but it is recorded here because the change that breaks it would be made in `script.js`, not in `toolkit.js`.
- **Deliberately not done:** the Python package does not filter on `status` — recorded as [GAP-08]. Neither `pytest` nor Pillow can be installed in this cloud session, and editing `cli.py`'s tool listing without being able to run `python/tests/` would trade a known small gap for an unknown one. Running `link` from Python already fails honestly, and the package is installed deliberately rather than stumbled onto, so the exposure is cosmetic.
- **Verified, not assumed:** `npm run check` green (376 tests). The `link.html` copy claims Python reports the tool as unimplemented rather than returning an empty list — that claim was checked by loading `registry.py` directly and calling it, which returned `ToolNotFound: Tool 'link' is declared but has no implementation installed.` and validated arguments to `{'source': 'a.md', 'scope': 'all', 'unique': True, 'format': 'list'}`. The three new behavioural tests were also mutation-checked: neutering `toolVisible()` to `return true` fails exactly those three, so they are not passing vacuously.
- **One thing the tests still cannot see:** `test/dev-mode.test.js` asserts the dev tool never reaches the live spec, but that assertion goes vacuous if `link` is ever the last `dev` tool and its status flips to `live`. It was left that way rather than forcing a permanent dev tool to exist, which would be a worse constraint than the one it protects against.
...
- **Trigger Event:** AI Edit
- **Author/Agent:** Claude (Claude Code) for 06pratyush
- **Target Subsystem:** `toolkit.js`, `spec/manifest.json`, `test/dev-mode.test.js`, `data.html`
- **Intent:** Remove the tool-status filter added in the previous entry. Hiding unfinished tools from visitors is no longer a project goal — stated directly by 06pratyush — so `status` goes back to being a label describing how finished a tool is, and stops being an access control.
- **Bugs/Gaps Addressed:** None. This reverses a decision rather than fixing a defect. [GAP-08] is rewritten rather than closed: it previously recorded that the website filtered on `status` and the Python package did not, and that inconsistency is gone because **neither** filters now.
- **Context Modifications:**
  - `toolVisible()`, `visibleTools()`, `MODE_KEY` and `readMode()` deleted from `toolkit.js`, and `loadManifest()` returns the fetched manifest untouched again. `toolkit.js` is now byte-identical to its state before the previous entry — verified with `git diff origin/main -- toolkit.js`, which is empty, rather than assumed from the edits.
  - The six tests covering the filter are removed from `test/dev-mode.test.js`. The suite is back to 370 tests. Everything protecting the *page and nav* dev split is untouched — the CSS default-hide, the toggle, the touch gesture, the "every navbar carries the same dev group" check and `DEV_ONLY_PAGES` all still stand. Only the tool half is gone.
  - The manifest `$comment` is rewritten. It previously promised that `"dev"` tools are "visible only in dev mode and are never shown to a visitor", which the removal makes false — and a lie in the contract file is worse than no comment, since it is the first thing anyone adding a tool reads. It now says `status` is a label, that nothing filters on it, and that a `"dev"` entry must therefore describe itself honestly.
  - The `link` tool's `summary` now ends "Not built yet - the arguments are declared, the implementation is not." That sentence is doing real work: with no filter, the summary is visitor-facing.
  - `data.html`'s spec row for the tool changes from "Visible to: Dev mode only" to "Listed in: The assistant, like any other tool", because the first is now simply untrue.
- **The consequence, recorded because it is not visible in the diff:** with the filter gone, a visitor who types "extract links from this file" is understood, routed, and told `link` "runs in the Python package rather than the page", then handed `pip install "thehallucinatedlab[rag]"` and `thl link ...`. That command exits with `ToolNotFound`. This was raised with 06pratyush before the removal and the removal was confirmed, so it is a known accepted cost, not an oversight. It is written into [GAP-08] so the next agent does not rediscover it as a bug.
- **What is now unprotected:** the honesty of a `"dev"` tool rests entirely on its own prose, and no test can check prose. The previous entry had a mechanism; this one has a convention. That trade is the point of the change, but it is worth naming — the failure mode is a future `"dev"` tool with a confident summary shipping a promise that nothing keeps.
