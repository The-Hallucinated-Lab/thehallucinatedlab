...
- **Verification:** `npm run check` — eslint clean, 377/377 tests pass, spec in sync. Not visually rendered this session; the change is one card block copied structurally from its neighbours with no CSS change, and the grid-orphan claim above is a geometry consequence of the card count rather than an observation.
