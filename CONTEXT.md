...
- [GAP-08]: `status: "dev"` is filtered on the website but not in the Python package. `toolVisible()` in `toolkit.js` keeps dev tools out of every browser surface, while `registry.names()` — and therefore the listing at `cli.py:63` — returns them unconditionally, so `thl` lists `link` alongside the implemented tools. Running it is honest (`ToolNotFound: declared but has no implementation installed`), and the package is installed deliberately by a developer rather than stumbled onto by a visitor, so this is a consistency gap rather than a leak. Left unfixed because neither `pytest` nor Pillow is installable in the cloud session that introduced it, and changing the CLI's tool listing without being able to run `python/tests/` would be trading a known small gap for an unknown one.
