...
| **Styling** | Plain CSS (`styles.css`, `pages.css`, `fonts.css`) | — | No preprocessor, no framework, no third-party origin. Fonts: Manrope (prose) + IBM Plex Mono (system layer), self-hosted; `scripts/font-metrics.py` produces the fallback overrides |
...
12. [RULE-12]: **The page chrome is generated, never hand-edited.** The `<header>` (navbar), the `<footer>`, the favicon links and the font preloads on every page come from one template per block in `scripts/sync-shell.js`. Edit the template and run `npm run shell:sync`; never edit those blocks in a page. CI runs `node scripts/sync-shell.js --check` and fails on any page that drifts.
...
- [GAP-04]: Page content is duplicated across `<head>` blocks (nav, CSP, footer). There is no templating layer. **Mostly closed:** `scripts/sync-shell.js` owns the `<header>` (navbar), the `<footer>`, the favicon links and the font preloads on every page and regenerates them from one template each; `--check` runs in `npm run check`. Still hand-pasted: the CSP meta, the description/og metas and the JSON-LD.
...
- [GAP-11]: Blog pages have two `<footer>` elements (`.blog-footer` inside `<main>`, then the site footer). `script.js`'s dev-mode triple tap binds to `document.querySelector('footer')`, so on a blog page it lands on the tag block, not the site footer. Pre-existing; recorded here rather than fixed inside the redesign.
- [GAP-12]: The homepage transfer budget in `test/site-invariants.test.js` names the six text files and three font files on the first load but not `theme.js`, which is also requested. Adding it takes the request count to 10 of 10 with no slack; the number is real either way.
- [GAP-13]: Only `index.html` has been given the identity's type scale and layout. Every other page wears the new tokens, fonts and shell but keeps its previous sizes and component shapes until its own pass.
...
- **Trigger Event:** AI Edit
- **Author/Agent:** Claude (Master Orchestrator) for 06pratyush
- **Target Subsystem:** `dictionary/index.html`, `dictionary/terms/*.html` (40 pages)
- **Intent:** Bring the dictionary pages' `Content-Security-Policy` back in line with the one every other page on the site ships. The dictionary shipped in PR #46 with three additions to the policy: `https://fonts.googleapis.com` on `style-src`, `https://fonts.gstatic.com` on `font-src`, and `https://thehallucinatedlab.space` on `img-src`. All forty pages now carry the site-standard policy verbatim.
- **Bugs/Gaps Addressed:** Closes a widening of [GAP-02]'s blast radius. The two Google Fonts origins were dead permission — nothing in `dictionary/assets/css/` contains an `@import`, an `@font-face`, or any reference to either host; the pages use the same self-hosted families as the rest of the site through `--font-body` / `--font-heading`. `script-src` was `'self'` throughout and is untouched, so nothing was ever executing off-origin. What this fixes is the standing rule, not a live exposure: RULE-01 says no third-party origin, and a policy that permits two is a rule that has quietly stopped being true.
- **Context Modifications:** The `img-src` origin was **not** dead, and that is the part worth recording. Every dictionary page declared its favicon as an absolute `https://thehallucinatedlab.space/assets/images/logo.jpeg`, which resolves same-origin in production but is a cross-origin load anywhere else, and which the `img-src` grant existed to permit. Removing the grant alone therefore broke the favicon — caught by loading the pages in Chromium, not by the suite, which has no assertion about absolute same-origin references. The fix was to make the reference relative (`../` from `dictionary/`, `../../` from `dictionary/terms/`) the way every other page already does it, after which `img-src 'self' data:` covers it everywhere. No stylesheet, script, image or dependency was added or removed; the rendered pages are byte-identical.
- **New gap recorded:** [GAP-08] — the dictionary's search is dead in production. `dictionary/assets/js/app.js:45` fetches `data/search-index.json`, and `dictionary/data/` has never existed in the repository, so the fetch 404s on every load, `init()` throws, and the search input renders `disabled` with the placeholder "Search unavailable — browse the sections below". This predates this change and is unrelated to it: a missing file 404s regardless of policy, and `connect-src 'self'` is unmodified. Left unfixed deliberately — generating and committing a search index is a feature, not a CSP cleanup, and folding it into this pull request would hide a real piece of work inside a one-line policy edit.
- **Deliberate omission:** No test was added asserting that the dictionary CSP matches the site standard. The existing third-party-origin test checks *subresource references*, not the policy text, which is exactly why this drifted in silently — but the right fix is one assertion comparing every page's policy against a single canonical string, and that is its own argued change affecting all 70-odd pages rather than a rider on this one.
- **Verification:** `npm run check` — eslint clean, 377/377 tests, spec in sync. Loaded `dictionary/index.html` and `dictionary/terms/transformer.html` in Chromium against a local server: zero CSP violations on both, versus a blocked favicon when the `img-src` origin was removed without the relative-path fix. The rendered `dictionary/index.html` is a byte-identical PNG to the same page built from unmodified `main`.
...
- **Trigger Event:** AI Edit
- **Author/Agent:** Claude (Master Orchestrator) for 06pratyush
- **Target Subsystem:** `script.js`, `dictionary/index.html`, `dictionary/terms/*.html` (40 pages), `dictionary/assets/css/dictionary.css`, `dictionary/assets/js/browse.js` (new), `dictionary/assets/js/nav.js`, `test/dictionary-browse.test.js` (new)
- **Intent:** Make the dictionary navigable. Six defects, each with a distinct root cause: the Dictionary nav entry rendered as a bare word instead of a glyph; the navbar on every dictionary page was unstyled; the alphabet threw the reader down a 7,163px document; the card grid was visibly scattered; there was no way to page through a corpus; and on a narrow screen the sticky search bar and the page content painted on top of each other.
- **Bugs/Gaps Addressed:**
  - **Nav glyph.** `NAV_ICONS` is keyed by the literal `href`, and `key()` only stripped a leading `./`. The same page is a different string from every depth, so an entry only got its icon where the table happened to match — Dictionary nowhere, and Sitemap not inside `/blogs`. `key()` now flattens `../` and a leading `/` to a root-relative path. Fixing Dictionary fixed Sitemap-in-blogs for free.
  - **Unstyled navbar.** Dictionary pages loaded `fonts.css`, `tokens.css` and `dictionary.css` but never `styles.css`, which is where every `.navbar` / `.nav-links` / `.theme-toggle` rule lives. They shipped the shared navbar markup with none of its CSS. All 40 pages now load it.
  - **Scattered grid.** Each letter ships an empty `<span id="letter-X">` before its first card as a jump target. They are direct children of `.corpus-grid`, so all fifteen were consuming grid cells — which is why the first row began in column two and no row ever lined up. They now span the row at zero height, and are hidden outright once enhanced.
  - **Scroll jump.** The alphabet was `<a href="#letter-A">` into a 7,163px page. Letters now filter the grid in place.
  - **Sticky overlap.** `.search-block` is sticky with `linear-gradient(..., var(--bg-primary) 72%, transparent)`. The fade began level with the hint line, so scrolled content showed through the bar and the two ran together as one block of overlapping text — worst at 390px, where the chips wrap and the hint runs to three lines. Now opaque to 94%, fading only across the bottom padding.
- **Context Modifications:** New `browse.js` (letter filter + pager, 12 per page) and `test/dictionary-browse.test.js` (13 cases). The paging and filtering decisions sit in a `@pure-start` block and are tested under node through the existing `load-pure` helper rather than a DOM harness. Controls are built in JS, not markup: the alphabet still ships as real anchors, every card is visible with JavaScript off, and that no-JS path is unchanged. Entry animation is a capped stagger honouring `prefers-reduced-motion`. `nav.js` lost its menu-toggle block — it bound `.nav-toggle`, which the markup does not contain (the button is `#nav-hamburger`), so the mobile menu on these pages was dead code competing with `script.js`, which already owns it site-wide. No new dependency, image or origin; the CSP is untouched.
- **Regression I introduced and then fixed, recorded because the diff hides it:** loading `styles.css` also imports its site-wide `section { padding: 6rem 2rem }`, written for the marketing pages. Term pages space themselves with a margin and never wanted it — inherited, it put 96px above and below every block and took a term page from 1,299px to 2,551px of mostly whitespace. `.term-section { padding: 0 }` restores it (1,334px; the 35px residue is base typography, not layout). The hub is unaffected because its own `.section` rule already sets that padding.
- **Deliberate omission — two mechanisms written and then removed.** Scroll position was first held by saving and restoring `window.scrollY` around each render, plus `overflow-anchor: none` on the grid. Both were written against a ~2,800px lurch that turned out to be the *test harness* scrolling the element into view before clicking it, not the page. Re-measured with a native click and a settle-until-stable helper, `preventDefault()` alone holds the offset across six positions including mid-document and at the bottom, and removing both changed nothing. They are not in the shipped file; the comment in `browse.js` records what was tried so the next agent does not re-add them. **[GAP-08] is untouched** — the search index is still missing and search is still disabled in production. Browsing is now good enough that the dead search is the most visible remaining flaw on the page.
- **Verification:** `npm run check` — eslint clean, 390/390 tests (377 + 13 new), spec in sync. In Chromium at 1440px and 390px: nav renders 11 icons on the hub and on a term page with Dictionary among them; both corpora filter and page correctly (21 → 2 pages, 18 → 2 pages, "E" → Eigenvector/Embedding/Entropy); scroll offset unchanged across six letter/page interactions; no console errors on a term page. Hub height fell from 7,163px to 5,166px.
...
- **Trigger Event:** AI Edit
- **Author/Agent:** Claude (Master Orchestrator) for 06pratyush
- **Target Subsystem:** `dictionary/assets/css/tokens.css`, `dictionary/assets/css/dictionary.css`, `dictionary/assets/js/browse.js`, `script.js`, `test/dictionary-browse.test.js`
- **Intent:** Finish the dictionary browse experience and make the navbar genuinely identical everywhere. Four things: the letter filter had to actually hide cards, the pager had to stay visible and reflect the filtered count, Back had to undo a filter, and the light-mode navbar had to stop rendering as a near-black bar on dictionary pages.
- **Bugs/Gaps Addressed:**
  - **The letter filter did not work in production, and the previous entry claiming it did was wrong.** `browse.js` hides a card with the `hidden` attribute, whose `display: none` comes from the UA stylesheet; any author `display` beats it, and `.entry-card` sets `display: block`. So filtering set the attribute on 18 of 21 cards, updated the count and the status line, and left all 21 painted. Every check written for it asserted on `el.hidden` — the property, not the rendered box — so a broken feature passed. `.corpus-grid .entry-card[hidden] { display: none }` fixes it, and a CSS assertion now guards it because no DOM-property test can.
  - **Light-mode navbar.** `tokens.css` carried a second, complete navbar implementation — `.navbar`, `.nav-inner`, `.nav-wordmark`, `.nav-toggle` and a ≤1024px block — written against markup these pages do not use, and loaded after `styles.css`, so it won. It hardcoded `background: rgba(5, 5, 5, 0.8)`, a literal the theme switch cannot reach, so light mode painted a near-black bar over a `#e8dfcb` page while `--bg-nav` sat correctly resolved at `rgba(232, 223, 203, 0.86)` and unused. `styles.css` carries a comment about fixing this exact bug once already. 149 lines removed; `styles.css` owns the navbar.
  - **Nav glyphs missing on five blog pages.** Their Media tab points at `../artifacts.html` or `../blogs.html`, so `NAV_ICONS['media.html']` never matched and the tab rendered as a bare word. Aliased. A sweep of all 70 navbar-bearing pages now reports zero links without a glyph.
- **Context Modifications:** The pager is never hidden — a control that vanishes on a three-entry result reads as a broken page, not as "this is all of them". Entries are sorted with `localeCompare` and painted through `order`, so the slice a page takes no longer depends on markup order. Each section owns a query parameter keyed by its id (`?ai-mathematics=E.2`), which makes Back undo a filter instead of leaving the dictionary, makes a narrowed view a shareable link, and survives reload. `decodeState` treats that parameter as hostile input: a malformed letter drops its page rather than honouring it, because that page was a position inside a filter that no longer exists.
- **Deliberate omission:** **[GAP-08] still open** — the search index is still missing and search is still disabled in production.
- **New gap recorded:** [GAP-09] — `dictionary/assets/css/tokens.css` still redeclares 23 design tokens that `styles.css` owns (`--bg-card`, `--gold-primary`, `--text-muted` and others), with no light-theme block of its own. It is currently harmless: `styles.css` scopes its light values as `:root[data-theme="light"]`, which outranks a bare `:root`. It stops being harmless the moment anyone raises the dictionary's specificity or adds a theme block here. The duplication should go; it was left alone in this change because removing 23 tokens is a visual-regression risk across 40 pages and deserves its own pull request.
- **Verification:** `npm run check` — eslint clean, 402/402 tests (25 in this file), spec in sync. Measured in Chromium by *rendered box*, not by attribute: filtering to "E" paints 3 of 21 cards. Nine-step flow passes — filter, re-filter, Back, Back, Forward, shared link, malformed URL falling back to the full corpus, and both corpora holding independent state. Navbar backgrounds now identical across hub, term page and homepage in both themes (`rgba(5,5,5,0.95)` dark, `rgba(232,223,203,0.97)` light). Mobile menu opens on dictionary and homepage alike.
...
- **Trigger Event:** AI Edit
- **Author/Agent:** Claude Code (Master Orchestrator)
- **Target Subsystem:** `scripts/sync-dictionary.js`, `package.json`
- **Intent:** Make bringing a dictionary rebuild into this repository a repeatable command instead of a hand-adaptation nobody can reproduce.
- **Bugs/Gaps Addressed:** The dictionary is generated in `06pratyush/ai_dictionary_thl` and was adapted for this site by hand at integration time — local stylesheet and font paths instead of Google Fonts, a per-page CSP meta, this site's navbar and footer, `theme.js`, descriptions truncated to the 155-character cap. None of that was written down as code, so an upstream rebuild could not be brought over without redoing all of it from memory, and that adaptation is exactly the kind of work that gets done differently the second time.
- **Context Modifications:** The script is deliberately surgical rather than a copy. It replaces only what the generator owns — `<title>`, the description and the og/twitter descriptions derived from it, the JSON-LD graph, and everything between `<main>` and `</main>` — and leaves the CSP, asset paths, navbar, `browse.js` and footer untouched. `dictionary.css` is **not** copied at all: this repo patches it in several places the upstream file does not contain, so only the delimited `topic-page-interface` block is replaced. That distinction is not theoretical — the first attempt copied the stylesheet wholesale, reverted the letter filter's `[hidden]` override, and was caught by `test/dictionary-browse.test.js`, which is precisely the failure that test was written for. `--check` mode reports staleness without writing, for use in review.
- **Verification:** `npm run check` — eslint clean, 402/402 tests, spec in sync. `node scripts/sync-dictionary.js --check ../ai_dictionary_thl` reports in sync.
...
- **Trigger Event:** AI Edit
- **Author/Agent:** Claude Code (Master Orchestrator)
- **Target Subsystem:** `dictionary/` (39 term pages, hub, stylesheet block), `llms.txt`, `llms-full.txt`
- **Intent:** Bring the dictionary onto the Topic Page Specification — one fixed structure for every term, so the reference reads as a reference rather than 39 pages of whatever each entry happened to carry.
- **Bugs/Gaps Addressed:** Term pages rendered five ad-hoc sections in an order that varied with the data. They now render the spec's sections in the spec's order under stable anchors (`#quick-take`, `#formal-statement`, `#deep-dive`, `#faq`, …), each with an on-page contents list. The hub cards carry the same vocabulary — category, difficulty, reading time — so the card and the page it opens agree. Attention Mechanism is fully authored against the spec as the proof case; the other 38 render derived Quick Take and Final Formal Statement blocks marked `data-derived="true"` and omit the sections they cannot honestly supply.
- **Context Modifications:** Structured data is now stricter, not looser. `FAQPage` is emitted only from FAQ pairs actually rendered on the page — RULE-05 in mechanical form — and `Article` carries the author and dates only where the entry records them. Three long headwords (Convolutional Neural Network, Maximum Likelihood Estimation, Principal Component Analysis) had hand-shortened titles here; that convention moved upstream into the generator, so the site no longer patches them. `llms.txt` and `llms-full.txt` describe the new page structure, per GAP-03 — the pages changed shape, so prose claiming otherwise would have drifted on the very first sync.
...
- **Trigger Event:** AI Edit
- **Author/Agent:** Claude Code (Master Orchestrator) for @06pratyush
- **Target Subsystem:** `styles.css` (tokens), `pages.css`, `blogs/*.css`, `dictionary/assets/css/tokens.css`, `dictionary/assets/css/dictionary.css`, six redirect stubs, `eda.js`, `blogs/complexity.js`, `converters*`, `test/regressions.test.js`
- **Intent:** Second PR of the redesign series — the colour system. The gold family becomes the THL palette: Charcoal `#0B0F14` / Ivory `#F4F1EA` carry the interface, Slate `#405A73` is structure, Saffron `#F28A3D` is the signal, Sage `#2E5B52` is the resolved state. The token family is renamed `--gold-*` → `--accent-*` (owner decision) because a token called gold holding orange is a trap for the next reader. New tokens: `--signal` (raw saffron, never text), `--structure`, `--ok/--warn/--error` with `-rgb` channels, `--logo-petal`, `--border-strong`, `--shadow-card`, `--bg-rgb`, and the motion scale (`--motion-*`, `--ease-*`, `--stagger-*`; `--transition` is kept as a compat shorthand). Radii tightened (6/10/14/18) so cards read as technical records.
- **Bugs/Gaps Addressed:** GAP-09 closed — `dictionary/assets/css/tokens.css` no longer redeclares the `:root` palette (it loaded after `styles.css` and would have pinned the dictionary to the old gold), nor its competing `.fade-in` keyframe and reduced-motion block; its unused `--status-*` tokens are gone. Thirty-odd literal `rgba(201, 168, 76, …)` washes, the near-black scrims, the status reds/greens and the dev-mode orange are now token expressions, so the light theme reaches them without the end-of-file patch list growing.
- **Context Modifications:** `test/regressions.test.js` mirrors the rename (`accent`, `accent-dark`) and checks two more text tokens (`ok`, `error`) at the same 4.5:1 bar — strictly more is asserted, nothing is loosened. Light-theme text saffron is a burnt `#9C4407` (raw saffron is 2.2:1 on ivory); the two-token split is documented at the top of `styles.css`. `blogs/complexity.css` declared `--accent: var(--gold-primary)` — a cycle after the rename — so its alias pair was removed and `--accent-2` uses point at `--accent-light`. `blogs/blog.css`'s hero scrim is pinned to charcoal channels in both themes: that hero is a deliberate dark island.
- **Deliberate omission:** Fonts, logo, navbar shape and the homepage are unchanged here; they follow in PRs 3–5. `blogs/complexity.css` keeps its own page-local status hues (`--cx-good/--cx-warn`) — they are chart semantics, not brand.
- **Verification:** `npm run check` — eslint clean, 423/423 tests, spec and shell in sync. Both themes checked in the preview on `/`, `/media.html`, `/blogs/sample-blog.html`, `/dictionary/terms/attention-mechanism.html`; no console errors. `./.orchestrator/verify.sh` → `VERIFY_PASS`.
...
- **Trigger Event:** AI Edit
- **Author/Agent:** Claude Code (Master Orchestrator) for @06pratyush
- **Target Subsystem:** `fonts.css`, `assets/fonts/`, `scripts/font-metrics.py` (new), `scripts/sync-shell.js` (font preloads), `styles.css` (type roles), `dictionary/assets/css/tokens.css`, `README.md`, `test/site-invariants.test.js`, `test/sync-shell.test.js`
- **Intent:** Third PR of the redesign series — typography. Outfit + JetBrains Mono give way to Manrope (display, headings, body, navigation; one variable file per subset) and IBM Plex Mono (labels, metadata, status, code, form controls; static 400 and 500). The mono face moves from being the body font to being the system layer, which is the brand's split: prose reads as prose, and anything that reads as a coordinate or a status is monospace. `styles.css` gains `--font-mono` and a TYPE ROLES block so a component says which role it is rather than which file.
- **Bugs/Gaps Addressed:** The 30 pages that preloaded the old font files by hand, and the 40 dictionary pages that preloaded nothing, now all carry the same three preloads, generated by `sync-shell.js` before the `fonts.css` link (root-absolute, like `fonts.css`'s own `src`). `dictionary/assets/css/tokens.css` had set its body to the mono face; it now inherits the prose face and only its search controls and hint are mono. First-load font bytes drop from 62 KB to 53 KB.
- **Context Modifications:** The fallback overrides in `fonts.css` are computed by `scripts/font-metrics.py` from the TTF builds with fontTools (fontTools cannot open WOFF2 here — no brotli — and the site keeps its Python footprint minimal on purpose); the README records the exact commands. `test/site-invariants.test.js`'s first-load list names the three new files (the old ones are deleted in the same commit). `gate.sh` sends mypy's cache outside the tree: the >1 MB tree walk in `regressions.test.js` caught `.mypy_cache` on the first run.
- **Deliberate omission:** The type *scale* (display 96, H1 72, label 11/0.08em) is not applied yet — sizes still follow the old rules and are tuned in the shell and homepage PRs where the layouts that carry them change. The old Google-Fonts URL in the README history is left as history.
- **Verification:** `npm run check` — eslint clean, 426/426 tests, spec and shell in sync. `document.fonts` in the preview reports Manrope 400–800, IBM Plex Mono 400 and 500 loaded; body/h1 resolve to Manrope, labels to Plex Mono, checked on `/tools.html` and `/dictionary/index.html` in both themes. `./.orchestrator/verify.sh` → `VERIFY_PASS`.
...
- **Trigger Event:** AI Edit
- **Author/Agent:** Claude Code (Master Orchestrator) for @06pratyush
- **Target Subsystem:** `assets/images/logo.svg` (new, the mark), `scripts/render-logo.py` (new), `scripts/sync-shell.js` (header/footer/favicon templates), all 70 navbar pages, `styles.css` (navbar, lotus, icon bar, footer, mobile panel, forced-colors), `script.js` (NAV_ICONS, lotus assembly), `toolbench.js`, `pages.css`, `dictionary/assets/css/dictionary.css`, `test/site-invariants.test.js`, `test/sync-shell.test.js`, `README.md`, `CONTRIBUTING.md`
- **Intent:** Fourth PR of the redesign series — the identity and the shell. The lotus is now a hand-authored SVG (side view: three soft-gray petals and one saffron sliver from a single convergence point, `viewBox 0 0 256 256`, four absolute-path petals) inlined on every page so its petals can be styled by the theme and animated. The navbar is a centred floating pill: a fixed transparent strip positions it, the blur lives on a pseudo-element, the nav glyphs are a stroked line set drawn in the same hand as the mark, resting glyphs are muted ink and the current section is saffron. The footer is the identity's: the mark large and monochrome, the wordmark, what the lab is, where to go, who owns it. The mobile menu is a controlled panel hanging under the pill (400 ms, 40 ms stagger) rather than a full-screen takeover.
- **Bugs/Gaps Addressed:** GAP-04 mostly closed (footer, favicons and preloads are generated now, not only the header). The dictionary's 1024-px JPEG favicon and its 1200×630 `og:image` size for a square file are corrected by the generator. `toolbench.js` hosted its codeview toggle on `.navbar`, which is no longer the pill — it now hosts it beside the theme toggle wherever that lives. The `.navbar:has(.nav-links.open) { backdrop-filter: none }` workaround is gone: the blur is on `.nav-pill::before`, so the pill is never a containing block for the panel.
- **Context Modifications:** `logo-72.*` and the unreferenced `logo-72-light.*` are deleted; `logo.jpeg`, `favicon-32.png` and `favicon-180.png` are re-rendered from the SVG by `scripts/render-logo.py` (Pillow, supersampled) and keep their names, so the 148 `og:image` references stay valid. The homepage transfer list in `test/site-invariants.test.js` drops `logo-72.avif` — no image is on the first-load path any more. Motion is gated: the lotus assembles once per session (`sessionStorage`), only when `shouldAnimate()` allows; hover fans the petals 2–4 px; reduced motion collapses both. Forced-colors hands the petal fills and glyph strokes to `CanvasText`/`Highlight`.
- **Deliberate omission:** The homepage body is still the old one (hero, explore cards, team) and follows in PR 5. The 14B `auditor` spills to CPU on this machine and a file audit takes over ten minutes, so audits of the new scripts run in the background; the harness's `audit.sh`/`moe.sh` were fixed in passing to refuse to report CLEAN when an auditor returns nothing — the first run had done exactly that.
- **Verification:** `npm run check` — eslint clean, 429/429 tests, spec and shell in sync. Preview at 1280 (both themes): pill centred, glyphs stroked, active state saffron, footer as designed, codeview toggle inside the pill on `/convert.html`, dictionary term page unaffected. Mobile 375: panel opens under the pill, not clipped; Escape closes and refocuses the burger. No console errors. `./.orchestrator/verify.sh` → `VERIFY_PASS`.
...
- **Trigger Event:** AI Edit
- **Author/Agent:** Claude Code (Master Orchestrator) for @06pratyush
- **Target Subsystem:** `index.html` (rebuilt), `home.css` (new), `home.js` (new), `script.js` (particles and typing removed), `styles.css`, `pages.css`, `eslint.config.js`, `test/site-invariants.test.js`, `test/home.test.js` (new), `llms-full.txt`
- **Intent:** Fifth PR of the redesign series — the homepage. The page now follows the identity's structure and says only what is true of this site: a hero with the positioning statement ("We build AI where it makes sense.") beside the mark at 360 px; "How the lab works" as five stages on one track with the saffron signal travelling through them; a Software / ML / AI selector (a real tab list) explaining which kind of system a problem needs; six dossier rows, one per section of the site; "Private by design" with a browser-only architecture diagram whose nodes light in sequence; the three founders; and a contact call to action. The hero mark answers the pointer by a few pixels and resolves into its own geometry as the reader scrolls — user-controlled, transform and opacity only.
- **Bugs/Gaps Addressed:** The particle canvas and the typing effect are gone — the identity forbids both, and removing them takes `script.js` from 32 KB to 23 KB. The homepage no longer loads `pages.css` (it used only the hero/explore/member rules, now in `home.css`), so its first load drops from 110 KB to roughly 92 KB. `llms-full.txt`'s homepage section had drifted twice over ("six cards", "two founders"); it now describes the page that ships.
- **Context Modifications:** `home.js` carries a `@pure-start`/`@pure-end` block (`petalOffset`, `lotusPhase`, `nextTabIndex`) tested under Node; everything else in it is gated by `shouldAnimate()` and the pointer work by `(hover: hover) and (pointer: fine)`. The first-load list in `test/site-invariants.test.js` now names `home.css` and `home.js` in place of `pages.css` (6 text + 3 font files = 9 requests of the 10 allowed). `eslint.config.js` no longer declares `initParticles`/`initTypingEffect` as globals.
- **Deliberate omission:** No case studies were invented. The "What's here" rows describe the six existing sections; the "Selected work" language of the brand document was not used because the site has no client work to show, and a dossier of fabricated engagements on a lab that argues for evidence would be self-refuting. The type scale is applied on this page only; the other pages keep their sizes until their own pass.
- **Verification:** `npm run check` — eslint clean, spec and shell in sync, all tests green. Preview at 1280 in both themes: hero, signal path (track fills, nodes light), selector (tab switch updates `aria-selected`, panels crossfade), dossier, privacy diagram, team, contact, footer; no horizontal overflow; no console errors. The hero's scroll phase was checked at `scrollY` 753 of an 800 px hero: `--phase` 1.000, geometry fully drawn. `./.orchestrator/verify.sh` → `VERIFY_PASS`.
...
- **Trigger Event:** AI Edit
- **Author/Agent:** Claude Code (Master Orchestrator) for @06pratyush
- **Target Subsystem:** `README.md`, `ARCHITECTURE.md`, `.github/workflows/ci.yml`, `DESIGN-LANGUAGE.txt` (local), this file (§3 RULE-12, §4 GAP-11/12/13)
- **Intent:** Sixth and last PR of the redesign series — the paper trail. The README's feature and design-system sections describe the site that ships now (the lotus, the pill, the colour system with its two-saffron rule, motion tokens, the type roles) instead of particles, gold and Outfit; the file tree names `home.css` / `home.js`; the Home and Navbar rows are rewritten. `ARCHITECTURE.md`'s budget note gives the new number. CI runs `node scripts/sync-shell.js --check` after lint, so a hand-edited navbar on any page fails the build. `DESIGN-LANGUAGE.txt` is rewritten as the identity's source of truth — colour, type, the lotus, motion tokens, components, breakpoints, accessibility — with every value read from the stylesheets.
- **Bugs/Gaps Addressed:** README badges carried the old gold hexes; the "preconnect hints for the font origins" line was stale since the fonts went same-origin. RULE-12 records that the chrome is generated. GAP-11 (blog double footer and the dev-tap target), GAP-12 (`theme.js` absent from the budget list) and GAP-13 (only the homepage carries the type scale) are recorded rather than quietly left.
- **Context Modifications:** None to the site itself in this PR.
- **Deliberate omission:** `DESIGN-LANGUAGE.txt` stays gitignored, as it was; it is the owner's local reference and the README points at it. GSAP-driven blog reveals were not re-timed to the motion tokens.
- **Verification:** `npm run check` — eslint clean, 449/449 tests, spec and shell in sync. `./.orchestrator/verify.sh` → `VERIFY_PASS`.
