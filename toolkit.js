...
(function () {
  ...
  function runImageConvert(file, rawArgs, manifest) {
    var tool = findTool(manifest, 'image_convert');
    if (!tool) return Promise.reject(ToolError('image_convert is not in the tool spec.'));
  ...
  function run(name, file, args, manifest) {
    if (name === 'image_convert') return runImageConvert(file, args, manifest);
